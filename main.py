...
def train_model(seq_length,kind_size,vocab_size,epochs=100,batchsize=100):
	"""
	epochs:训练轮数
	"""
	...
	with tf.Session() as sess:
		with tf.variable_scope("model",reuse=None):
			m=Model(kind_size,vocab_size,seq_length,batchsize=batchsize,init_lr=0.002)
		...
		for epoch in range(epochs):
			...
			for kind_test,desc_test in get_data(batchsize,kind_size,seq_length,'test',False):
				result=m_test.result.eval(feed_dict={m.input_kind:kind_test,m.input_content:desc_test})
				val=''
				for v in result:
					val+=str(v)+' '
				write_log(val,epoch)
...
if __name__ == "__main__":
	...
	if not os.path.exists("data/test.json"):
		store_data("test")
	train_model(seq_length,kind_size,vocab_size)
